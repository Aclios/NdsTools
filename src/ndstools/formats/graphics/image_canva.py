...
class ImageCanva:
    """
    The ImageCanva object allows to load 2D Graphics objects (bitmap, palette, cell, tilemap) and build images from them.

    To build a image, you need at least a bitmap AND a palette. You also might need either a tilemap OR a cell.

    If you are not working with the standard Nitro files (NCGR, NCLR, NCER, NSCR), you will also have to specify other parameters (image size, bit depth, linearity, OAM size).
    """
    ...
    def load_bitmap(self, bitmap: Bitmap):
        """
        Load a Bitmap object to the Canva, and set its parameters (bit depth, image size, linearity) if it's a NCGR file.
        """
        self.bitmap = bitmap
    ...
    def load_tilemap(self, tilemap: Tilemap):
        """
        Load a Tilemap object to the Canva, and set im size if it's a NSCR file.
    ...
    def load_palette(self, palette: Palette):
        """
        Load a Palette object to the Canva, and set bit depth if it's a NCLR file.
    ...
    def import_image(self, im_filepath: str, cell_idx: int | None = None):
        """
        Import an image (with a palette) to the canva and update the loaded objects. If there is a NCER file loaded, you must specify the idx of the cell you want to import.
        """
        im = Image.open(im_filepath)
        assert im.mode == "P", "Invalid png type, expected a color indexed png."
        assert (
            self.bitmap is not None and self.palette is not None
        ), "At least a palette and a bitmap are required"
        if self.cell is not None:
            if self.cell_idx is None:
                raise Exception("You must specify cell index if a NCER file is loaded.")
    ...
    def _import_cell(self, im: Image.Image, cell_idx: int):
        ...
        """
        if cell_idx >= len(self.cell.cebk.cells):
            raise Exception(
                f"Given cell index is too high: cell with indec {cell_idx} doesn't exist."
            )
    ...
    def images(self):
        """
        The images generated by combining the object of the canvas.

        There will only be 1 image, unless a NCER file was loaded. In that case, there will likely be several images, and some of them could be None, since a NCER cell can be empty.
        """
        if not self._images:
            raise Exception("No images found. Did you call .resolve() before?")
        return self._images
